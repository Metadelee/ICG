...
bool SimpleRasterizer::Render(Image &image, const Scene &scene)
{
  ...
  ScanObject(&scene);
	...
	mat4x4 viewTransform = glm::inverse(camera->GetGlobalTransformation()); // not sure if this is the right one (probably not)
	mat4x4 projectionTransform = glm::perspective(camera->GetFov(), camera->GetAspect(), camera->GetNearClip(), camera->GetFarClip());
